...
def PlotTerminalPiers(bcn, nombre_terminal):
    ...
    max_gates = max([len(area.gates) for area in terminal.boarding_areas])
    max_levels = (max_gates + 1) // 2

    ESPACIO_MINIMO_VERTICAL = 1.0
    PIER_HEIGHT = max(10.0, (max_levels + 1) * ESPACIO_MINIMO_VERTICAL)
    Y_MAIN_CONCOURSE = PIER_HEIGHT + 2.0

    GATE_STICK_LEN = 2.8
    STATUS_BOX_W = 0.5
    STATUS_BOX_H = 0.25

    conc_width = max(16.0, (num_bas + 1) * 5.0)
    conc_height = 0.8
    conc_x = 0
    conc_y = Y_MAIN_CONCOURSE - (conc_height / 2)

    fig_width = max(14, int(conc_width * 0.9))
    fig_height = max(8, int(PIER_HEIGHT * 0.8))
    ...
    for i, area in enumerate(terminal.boarding_areas):
        ...
        ax.text(pier_x_center, pier_y_bottom - 0.8, label_text, fontsize=12, ha='center', va='top',
                fontweight='bold')
        ...
        for g_idx, gate in enumerate(area.gates):
            ...
            ax.plot([stick_start_x, stick_end_x], [gate_y, gate_y], color='black', linewidth=1.2, zorder=1)
            ...
            ax.text((stick_start_x + stick_end_x) / 2, gate_y + 0.08, gate.name,
            ...
            if gate.occupied:
                ...
                if on_right_side:
                    ax.text(box_x + STATUS_BOX_W + 0.1, gate_y, ac_id, ha='left', va='center', fontsize=8,
                            fontweight='bold')
                else:
                    ax.text(box_x - 0.1, gate_y, ac_id, ha='right', va='center', fontsize=8, fontweight='bold')

    ax.axis('off')
    ax.set_title(f"Live Terminal Map - {bcn.code} ({terminal.name})", fontsize=16, fontweight='bold', pad=20)
    fig.tight_layout()
    return fig
